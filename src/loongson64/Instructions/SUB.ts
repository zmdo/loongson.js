...
namespace LoongarchTemplate {
     ...
     */
    export function SUB ( _LRG:LoongsonRegisterGroup, tag:number, rd:number, rj:number, rk:number ) : void {
        ...
        switch (tag) {
            case W : // 32位减法运算
            {
                let tmp = (GRj.slice(31,0).sub(GRk.slice(31,0)));
                _LRG.GR[rd] = SignExtend (tmp.slice(31,0), 32 ,_LRG.GRLEN);
            }
                break;
            case D : // 64位减法运算
                _LRG.GR[rd] = GRj.sub(GRk);
                break;
        }
